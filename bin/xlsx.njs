...
if (!wb) {
    console.error(n + ": error parsing " + filename + ": empty workbook");
    process.exit(0);
}
...
function outit(o, fn) {
    if (fn) fs.writeFileSync(fn, o); else console.log(o);
}
...
switch (true) {
    case program.formulae:
        doit(function (ws) {
            return X.utils.sheet_to_formulae(ws).join("\n");
        });
        break;
    ...
    case program.json:
        doit(function (ws) {
            return JSON.stringify(X.utils.sheet_to_json(ws, jso));
        });
        break;
    ...
    default:
        ...
        } else doit(function (ws) {
            return X.utils.sheet_to_csv(ws, {FS: program.fieldSep, RS: program.rowSep});
        });
        break;
}
